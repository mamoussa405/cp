...
void number_of_ways(vb& columns, vb& diag1, vb& diag2, int row)
{
    if (row == 8)
    {
        ++ans;
    ...
    for(int x = 0; x < 8; ++x)
    {
        if (columns[x] || diag1[x+row] || diag2[x-row+7] || board[row][x] == '*') continue;
        columns[x] = diag1[x+row] = diag2[x-row+7] = true;
        number_of_ways(columns, diag1, diag2, row + 1);
        columns[x] = diag1[x+row] = diag2[x-row+7] = false;
    }
}
...
void solve(void)
{
    for(int i = 0; i < 8; ++i)
        for(int j = 0; j < 8; ++j)
            cin >> board[i][j];
    vb columns(8), diag1(15), diag2(15);
    number_of_ways(columns, diag1, diag2, 0);
    cout << ans << nl;
}
int main(void)
{
