...
void  number_of_ways(vb& columns, vb& diag1, vb& diag2, int n, int row)
{
    if (row == n)
    {
        ++ans;
    ...
    for(int x = 0; x < n; ++x)
    {
        if (columns[x] || diag1[x+row] || diag2[x-row+n-1]) continue;
        columns[x] = diag1[x+row] = diag2[x-row+n-1] = true;
        number_of_ways(columns, diag1, diag2, n, row + 1);
        columns[x] = diag1[x+row] = diag2[x-row+n-1] = false;
    }
}
void solve(void)
{
    int n;
    cin >> n;
    vb columns(n) , diag1(n*2-1), diag2(n*2-1);
    number_of_ways(columns, diag1, diag2, n, 0) ;
    cout << ans << nl;
}
int main(void)
{
