...
void bfs(vvc& a, char target)
{
    ...
    while (!q.empty())
    {
        ...
        q.pop();
        cout << curr << " ";
        for (int i = 0; i < sz(a[curr - 'A']); ++i)
        {
            if (!vis[a[curr - 'A'][i] - 'A'])
                q.push(a[curr - 'A'][i]),
                vis[a[curr - 'A'][i] - 'A'] = true;
        }
    }
    cout << endl;
}

void dfs(vvc& a, char target, vector<bool>& vis)
{
    cout << target << " ";
    for (int i = 0; i < sz(a[target - 'A']); ++i)
    {
        if (!vis[a[target - 'A'][i] - 'A'])
            vis[a[target - 'A'][i] - 'A'] = true,
            dfs(a, a[target - 'A'][i], vis);
    }
}

void dfs_with_stack(vvc& a, char target)
{
    stack<char> s;
    vector<bool> vis(a.size(), false);
...
void solve(void)
{
    ...
    // vector<bool> vis(tmp, false);
    // vis[target - 'A'] = true;
    // dfs(a, target, vis);
    dfs_with_stack(a, target);
}
