...
void solve(void)
{
   ...
   for(int i = 0; i < n; ++i)
	...
	int h;
	int w;
	if (_max_cor.first == (n / 2))
		h = (n/2) + 1;
	else
		h = (_max_cor.first > (n / 2) ) ? _max_cor.first : abs(n - _max_cor.first) ;
	if (_max_cor.second == (m / 2))	
		w = (m / 2) + 1;
	else
		w =	(_max_cor.second > (m / 2)) ? _max_cor.second : abs(m - _max_cor.second);
	cout << h*w << nl;
}
int main(void)
{
